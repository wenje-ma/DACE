...
式中 $C>0$ 为给定常数（示例中常取 $C=4$）；参数的有效取值范围为：$\theta>0$，$\rho\in(0,1)$，$\rho_*\in(0,1)$，$\tau\in(-\infty,+\infty)$。简单来说，$\theta$ 是式（2.2.7）对应的尺度参数；$\rho$ 为两个输入在 $|h|=1$ 时的相关系数；$\rho_*$ 为两个输入在 $|h|=1/\sqrt{C}$ 时的相关系数（例如当 $C=4$、$d=1$ 时，$\rho_*=\mathrm{Cor}[Y(0),Y(1/2)]$）。通过基础代数运算，上述任意一种相关形式对应的参数都可以用其余参数表示。研究者选择特定参数化形式，主要出于以下考量：便于参数解释、适配贝叶斯方法的先验设定，以及提升似然函数与相关函数的优化效果；3.6.3 节将进一步讨论这些问题。
...
除了是分段三次多项式外，$R(h\mid\psi)$ 在所有 $h\in\mathbb{R}$ 处导数连续；图 2.8 右侧列展示了当 $\psi\in\{0.5,1.0,10.0\}$ 时 $R(h\mid\psi)$ 的光滑性。该三次相关函数具有紧支集，因为只要输入 $x_1$ 和 $x_2$ 相距足够远，即满足 $|x_1-x_2|>\psi$，就有 $R(h\mid\psi)=0$。从数值计算角度，该性质意味着 $Z(x)$ 的相关矩阵会存在大量零元素，因此可采用稀疏矩阵方法完成矩阵求逆等运算。提前参阅 3.2 节中计算机试验的最优线性无偏预测，利用式 (2.2.12) 可得到三次样条插值预测子。
